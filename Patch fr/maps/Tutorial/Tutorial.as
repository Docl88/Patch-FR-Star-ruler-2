...
#section server
	...
	void spawnPlayerFleet() {
		auto@ leaderDsg = playerEmpire.getDesign("Vaisseau de guerre");
		auto@ sup1Dsg = playerEmpire.getDesign("Vaisseau à faisseau");
		auto@ sup2Dsg = playerEmpire.getDesign("Chasseur lourd");
		auto@ sup3Dsg = playerEmpire.getDesign("Vaisseau lance missile");
	...
	void spawnEnemyFleet(const vec3d& pos, const string& base = "Vaisseau de guerre", uint supBase = 25, bool aggro = false) {
		auto@ leaderDsg = enemyEmp.getDesign(base);
		auto@ sup1Dsg = enemyEmp.getDesign("Vaisseau à faisseau");
		auto@ sup2Dsg = enemyEmp.getDesign("Chasseur lourd");
		auto@ sup3Dsg = enemyEmp.getDesign("Vaisseau lance missile");
	...
	void postInit() {
		...
		spawnEnemyFleet(systemData[0].position + vec3d(700, 0, 250), "Cuirassé", 5);
		spawnEnemyFleet(systemData[0].position + vec3d(500, 0, 150), "Cuirassé", 5);

		//Defenses for phasite
		{
			auto@ enemySup = enemyEmp.getDesign("Vaisseau à faisseau");
...
class SpawnAttackFleets : DialogueAction {
	...
	void call() {
		for(uint i = 0; i < 3; ++i) {
			...
			tut.spawnEnemyFleet(pos, "Cuirassé", 5, aggro=true);
		}
	}
};
...
class CheckDestroyGate : ObjectiveCheck {
	bool check() {
		return getSystem(0).object.BasicVisionMask & getEmpire(1).mask == 0;
	}
};
...
class CheckBuildScout : ObjectiveCheck {
	bool check() {
		...
		if(hw.constructionCount != 0 && hw.constructionName[0] == "Eclaireur")
			return true;
		return false;
	}
};
...
class CheckScoutFinished : ObjectiveCheck {
	bool check() {
		for(uint i = 0, cnt = playerEmpire.fleetCount; i < cnt; ++i) {
			...
			if(ship !is null && ship.blueprint.design.name == "Eclaireur")
				return true;
		}
		return false;
	}
...
class CheckBuildCarrier : ObjectiveCheck {
	bool check() {
		Planet@ hw = playerEmpire.Homeworld;
		if(hw.constructionCount != 0 && hw.constructionName[0] == "Transporteur lourd")
			return true;
		return false;
	}
};
...
class CheckAddBeamships : ObjectiveCheck {
	bool check() {
		...
		for(uint i = 0, cnt = cons.groups.length; i < cnt; ++i) {
			if(cons.groups[i].dsg.name == "Vaisseau à faisseau")
				return true;
		}
		return false;
	}
...
class CheckFinishCarrier : ObjectiveCheck {
	bool check() {
		for(uint i = 1, cnt = playerEmpire.fleetCount; i < cnt; ++i) {
			Ship@ ship = cast<Ship>(playerEmpire.fleets[i]);
			if(ship !is null && ship.blueprint.design.name == "Transporteur lourd")
				return true;
		}
		return false;
	}
...
class CheckOrbitalPlaced : ObjectiveCheck {
	bool check() {
		Planet@ hw = playerEmpire.Homeworld;
		if(hw.constructionCount != 0 && hw.constructionName[0] == "Plateforme de défense")
			return true;
		return false;
	}
};
...
class CheckMoveFleet : ObjectiveCheck {
	bool check() {
		...
		for(uint i = 1, cnt = playerEmpire.fleetCount; i < cnt; ++i) {
			...
			if(ship is null)
				continue;
			if(ship.blueprint.design.name == "Plateforme de défense") {
				created = true;
			}
...
class CheckDesignEdit : GuiObjectiveCheck {
	bool check() {
		...
		if(tab.originalDesign is null)
			return false;
		return tab.originalDesign.name == "Transporteur lourd";
	}
};
...
class CheckDesignArmor : GuiObjectiveCheck {
	...
	bool start() {
		armor = 0;
		auto@ dsg = playerEmpire.getDesign("Transporteur lourd");
	...
	bool check() {
		...
		if(tab.originalDesign is null)
			return false;
		if(tab.originalDesign.name != "Transporteur lourd")
			return false;
...
class CheckDesignSaved : GuiObjectiveCheck {
	bool check() {
		return playerEmpire.getDesign("Transporteur lourd").revision > 1;
	}
};
...
class CheckTargetDefPlat : GuiObjectiveCheck {
	bool check() {
		...
		if(to.dsg is null)
			return false;
		return to.dsg.name == "Plateforme de défense";
	}
};
