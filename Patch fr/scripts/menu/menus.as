...
void init() {
	//Show the version
	@version = GuiText(null, Alignment(Right-260, Bottom-20, Right-4, Bottom));
	version.horizAlign = 1.0;
	version.text = format("Version: $1 ($2) $3 ($4)", GAME_VERSION, SCRIPT_VERSION, MOD_VERSION, MOD_REVISION);
	version.color = Color(0xaaaaaaaa);

	//Create logo
	@menu_logo = GuiSprite(null, Alignment(
		Left+0.5f-381, Top, Width=762, Height=178),
		Sprite(material::SmallLogo));
	...
	if(latestShot.length > 0 && fileExists(latestShot)) {
...
void draw() {
	...
	int height = size*material::MenuTopBar.size.height;
	recti pos = recti_area(vec2i(0, -(height-120)), vec2i(screen.x, height));
	material::MenuTopBar.draw(pos);

	height = size*material::MenuBotBar.size.height;
	pos = recti_area(vec2i(0, screen.y-80), vec2i(screen.x, height));
	material::MenuBotBar.draw(pos);
